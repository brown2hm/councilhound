...
\author{CouncilHound --- impact analysis subsystem}
...
and Homeownership series \cite{hvs}. Household size uses a multifamily
bedroom-mix demographic multiplier in the tradition of Listokin et
al.~\cite{listokin2006} ($\bar{s} = 1.8$, range 1.5--2.2, for a typical
studio/1BR/2BR mix): the ACS B25010 tenure-level renter average
...
consistent with the Engel gradients in the CE income-quintile tables:
...
city does levy. (PPTRA car-tax relief is a fixed state block grant, so
...
sales tax follow average daily membership, so a new student brings that
revenue with them; only the local share is a cost to city taxpayers).
\begin{align}
...
component is the tax \emph{increase} when the current tax is known). The
school term appears unscaled in both framings: pupils generate tuition-contract
...
\assum{avg\_vehicle\_assessed\_value} & \$14{,}000 & \$9k--\$20k & NoVA personal-property rolls (estimate) \\
...
\item \textbf{Household size (addressed).} The unit-mix-blind ACS renter
      ...
      \emph{Residual:} the multiplier assumes a typical studio/1BR/2BR mix;
...
\item \textbf{CES income scaling (addressed).} Category spending scales by
      ...
      CE quintile gradients, not locally estimated.
...
FY2026 budget announcement (FCPS tuition contract and projected ADM, for the
school-split cost model); Census ACS 5-year (B01003,
