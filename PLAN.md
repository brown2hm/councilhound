...
**Cost note:** ~24 months × Council + PC ≈ 100–150 meetings × 2–3 h audio. Hosted API with diarization ≈ $50–180 for the full backfill; local is free but slower and still needs pyannote for speakers.
**Decision (2026-07-11):** transcription runs locally on the dev Mac via mlx-whisper (Apple Silicon GPU, whisper-large-v3-turbo, ~10x realtime → backfill is an overnight-to-days job, acceptable). faster-whisper remains the portable/Docker backend. Diarization is deferred to a later pass (schema already has speaker columns); revisit hosted APIs then. Canceled meetings are excluded from transcription — their placeholder clips (title card + music) make whisper hallucinate.
