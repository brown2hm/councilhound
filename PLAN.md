...
**Task:** For every `documents` row, extract raw text (PDF → text via pymupdf, OCR fallback for scanned PDFs). Transcribe every meeting's audio with faster-whisper (no captions exist — this is required, not a fallback). Populate `raw_text` and `transcript_chunks` with timestamps.
**Definition of done:** Every fetched document/audio file has usable extracted text; spot-check 5 meetings by hand for extraction and transcription quality. Transcription is resumable — a killed run picks up where it left off, never re-transcribes a finished meeting.
**Cost note:** ~24 months × Council + PC ≈ 100–150 meetings × 2–3 h audio. Local faster-whisper on Apple Silicon is free but slow (days of compute); a hosted API (~$0.006/min) is roughly $100–160 for the full backfill. Either is acceptable — pick based on wall-clock patience.
